...
import { formatNumber } from '../core/NumberFormatter.js?v=ff2c227';
...
export class FeedbackPanel {
  ...
  constructor(getContextFn) {
    ...
    const tokenMeta = document.querySelector('meta[name="feedback-token"]');
    const repoMeta  = document.querySelector('meta[name="feedback-repo"]');
    this.#token = tokenMeta?.content ?? null;
    this.#repo  = repoMeta?.content  ?? null;

    if (!this.#token || this.#token === 'dev') this.#token = null;
  }
  ...
  init() {
    ...
    // Populate game context on open (done in #open)
  }
  ...
  async #submit() {
    ...
    try {
      if (this.#token && this.#repo) {
        const url = await this.#postToGitHub(issueTitle, issueBody, type);
        this.#showStatus('success', `✅ Issue filed! <a href="${url}" target="_blank" rel="noopener">View on GitHub ↗</a>`);
      } else {
        this.#fallbackOpen(issueTitle, issueBody);
        this.#showStatus('success', '↗ Opened GitHub in a new tab — please submit the pre-filled issue.');
      }
      ...
      this.#showStatus('error', `Failed to submit: ${err.message}. Try again later.`);
    } finally {
      if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = 'Submit'; }
    }
  }

  async #postToGitHub(title, body, type) {
    const labels = ['user-feedback', this.#typeToLabel(type)];
    ...
    if (!resp.ok) {
      ...
      throw new Error(`GitHub API ${resp.status}: ${txt.slice(0, 120)}`);
    }
  ...
  #fallbackOpen(title, body) {
    if (!this.#repo) return;
    const type = document.getElementById('feedback-type')?.value ?? 'feedback';
    const labels = `user-feedback,${this.#typeToLabel(type)}`;
    // URL-safe truncation (GitHub query string limit is generous but cap at 4000 chars body)
    const safeBody = body.length > 4000 ? body.slice(0, 4000) + '\n\n_(truncated)_' : body;
    const url = `https://github.com/${this.#repo}/issues/new`
      + `?title=${encodeURIComponent(title)}`
      + `&body=${encodeURIComponent(safeBody)}`
  ...
  #captureGameContext() {
    ...
    // Upgrades with level > 0
    const upgrades = ctx.upgrades
      ? Object.entries(ctx.upgrades).filter(([, s]) => (s.level ?? 0) > 0)
      : [];
    if (upgrades.length) {
      lines.push('**Purchased Upgrades:**');
      for (const [id, s] of upgrades) {
        lines.push(`- ${id}: level ${s.level}`);
      }
      lines.push('');
    }
  ...
  #typeLabel(type) {
    return { bug: 'Bug', feature: 'Feature Request', feedback: 'Feedback' }[type] ?? 'Feedback';
  }
  ...
  #typeEmoji(type) {
    return { bug: '🐛', feature: '✨', feedback: '💬' }[type] ?? '💬';
  }
}
